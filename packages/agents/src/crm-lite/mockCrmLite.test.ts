...
describe('MockCrmLite — happy path', () => {
  it('creates company, contact, opportunity and reads them back', () => {
    const crm = makeCrm();
    const company = crm.upsertCompany({ workspaceId: WS, companyName: 'Budget Wheels', attributes: { region: 'BC', businessType: 'auto_dealership' } });
    const contact = crm.upsertContact({ workspaceId: WS, prospectId: PROSPECT, companyId: company.id, role: 'General Manager', emailExample: 'gm@budgetwheels.example' });
    const opp = crm.upsertOpportunity({ workspaceId: WS, prospectId: PROSPECT, companyId: company.id, stage: 'appointment_set', appointmentRef: 'appt-1', crmRecordRef: 'rec-1' });
...
describe('MockCrmLite — idempotency (double-upsert = single record)', () => {
  it('company: same key twice keeps one record with same id', () => {
    const crm = makeCrm();
    const a = crm.upsertCompany({ workspaceId: WS, companyName: 'Budget Wheels', attributes: { region: 'BC' } });
    const b = crm.upsertCompany({ workspaceId: WS, companyName: 'Budget Wheels', attributes: { region: 'AB' } });
  ...
  it('contact: same workspace+prospect twice keeps one record', () => {
    ...
    const a = crm.upsertContact({ workspaceId: WS, prospectId: PROSPECT, companyId: c.id, role: 'GM' });
    const b = crm.upsertContact({ workspaceId: WS, prospectId: PROSPECT, companyId: c.id, role: 'Owner' });
    expect(b.id).toBe(a.id);
  ...
  it('opportunity: same workspace+prospect+appointmentRef twice keeps one record', () => {
    const crm = makeCrm();
    const c = crm.upsertCompany({ workspaceId: WS, companyName: 'Budget Wheels' });
    const a = crm.upsertOpportunity({ workspaceId: WS, prospectId: PROSPECT, companyId: c.id, stage: 'lead', appointmentRef: 'appt-1' });
    const b = crm.upsertOpportunity({ workspaceId: WS, prospectId: PROSPECT, companyId: c.id, stage: 'proposal', appointmentRef: 'appt-1' });
    expect(b.id).toBe(a.id);
  ...
  it('opportunity: a different appointmentRef is a distinct record', () => {
    const crm = makeCrm();
    const c = crm.upsertCompany({ workspaceId: WS, companyName: 'Budget Wheels' });
    crm.upsertOpportunity({ workspaceId: WS, prospectId: PROSPECT, companyId: c.id, stage: 'lead', appointmentRef: 'appt-1' });
    crm.upsertOpportunity({ workspaceId: WS, prospectId: PROSPECT, companyId: c.id, stage: 'lead', appointmentRef: 'appt-2' });
  ...
  it('preserves createdAt across re-upsert and bumps updatedAt', () => {
    ...
    const crm = createMockCrmLite({ now: () => new Date(times[Math.min(t++, times.length - 1)]!), newId: seqIds('id') });
...
describe('MockCrmLite — blocked / rejected lifecycle via timeline', () => {
  it('records a blocked compliance event without creating CRM records', () => {
    const crm = makeCrm();
    crm.timeline.record({ workspaceId: WS, prospectId: PROSPECT, kind: 'compliance', outcome: 'blocked', summary: 'Prospect is do-not-contact' });
  ...
  it('records a rejected approval event', () => {
    const crm = makeCrm();
    crm.timeline.record({ workspaceId: WS, prospectId: PROSPECT, kind: 'approval', outcome: 'rejected', summary: 'Human rejected outreach' });
...
describe('MockCrmLite — PII safety', () => {
  it('rejects a non-reserved contact email', () => {
    ...
    const c = crm.upsertCompany({ workspaceId: WS, companyName: 'Budget Wheels' });
    expect(() =>
      crm.upsertContact({ workspaceId: WS, prospectId: PROSPECT, companyId: c.id, emailExample: 'gm@realdealer.com' }),
  ...
  it('allows a reserved .example contact email', () => {
    const crm = makeCrm();
    const c = crm.upsertCompany({ workspaceId: WS, companyName: 'Budget Wheels' });
    const contact = crm.upsertContact({ workspaceId: WS, prospectId: PROSPECT, companyId: c.id, emailExample: 'gm@budgetwheels.example' });
  ...
  it('no stored CRM record or timeline event contains raw-looking PII', () => {
    const crm = makeCrm();
    const company = crm.upsertCompany({ workspaceId: WS, companyName: 'Budget Wheels', attributes: { region: 'BC' } });
    crm.upsertContact({ workspaceId: WS, prospectId: PROSPECT, companyId: company.id, role: 'General Manager', emailExample: 'gm@budgetwheels.example' });
    crm.upsertOpportunity({ workspaceId: WS, prospectId: PROSPECT, companyId: company.id, stage: 'appointment_set', appointmentRef: 'appt-1' });
    crm.timeline.record({ workspaceId: WS, prospectId: PROSPECT, kind: 'crm_writeback', outcome: 'ok', summary: 'CRM record written (mock)', refs: { crmRecordRef: 'rec-1' } });
