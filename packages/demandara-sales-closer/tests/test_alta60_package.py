...
GOOD={'tenant_id':'budget_wheels_demo','lead_id':'bw-demo-001','email':'buyer@example.test','company':'Budget Wheels Demo','budget_cad':14500,'vehicle_interest':'2018 Toyota Corolla','consent_to_process':True}
APPROVAL={'status':'APPROVED','approval_receipt_hash':'proof-ad3a7716e67c8b48'}
...
class Alta60PackageIntegrationTests(unittest.TestCase):
    ...
    def test_crm_fake_adapter_v2_dry_run_payloads(self):
        out=crm_fake_adapter_v2(GOOD,'proof-ad3a7716e67c8b48','budget_wheels_demo')
        self.assertEqual(out['status'],'DRY_RUN_OK')
        self.assertFalse(out['live_crm'])
        self.assertFalse(out['network_attempted'])
        self.assertEqual(out['writeback_intent']['idempotency_key'], idempotency_key('budget_wheels_demo','bw-demo-001','proof-ad3a7716e67c8b48'))
        self.assertIn('fake_hubspot_payload', out)
        self.assertIn('fake_salesforce_payload', out)
        self.assertEqual(out['proof_receipt_reference'], 'proof-ad3a7716e67c8b48')
    def test_crm_fake_adapter_v2_blocks_tenant_and_bad_receipt(self):
        self.assertEqual(crm_fake_adapter_v2({**GOOD,'tenant_id':'other'},'proof-ad3a7716e67c8b48','budget_wheels_demo')['status'],'BLOCKED')
