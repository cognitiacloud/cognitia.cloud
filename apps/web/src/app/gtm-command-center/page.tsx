/**
 * `/gtm-command-center` — the canonical, integrated GTM Command Center.
 *
 * One screen that proves the B1–B6 mock GTM system works end-to-end from real
 * `@cognitia/agents` module output, plus a code-computed Alta implementation-
 * parity scorecard.
 *
 ...
 * (`lib/server/gtmCommandCenterData.ts`), which runs the REAL B1–B6 lanes and the
 * PR #159 integration packet (`assembleIntegratedRunPacket` /
 * `verifyIntegratedRunPacket`). No client component imports `@cognitia/agents`,
 * and there is NO hand-authored mirror of lane logic — every value below is real
 * module output.
 *
  ...
  return (
    <main style={{ maxWidth: 1040, margin: '0 auto', padding: 24, lineHeight: 1.55 }}>
      ...
      <p style={muted}>
        ...
        release gates → proof/attribution. Tenant <code style={mono}>{view.workspaceId}</code>{' '}
        (sandbox). Every value is mock and PII-safe; there is no live send, no real CRM write, and
        no send/call/SMS/WhatsApp/ad control on this page.
      </p>
