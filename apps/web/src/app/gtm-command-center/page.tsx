 ...
 *   scorecard.
 *
 * Async server component, no client IO. The data is the REAL output of the
 * `@cognitia/agents` B1–B6 modules, assembled by the server-only adapter
 * `lib/server/gtmCommandCenterData.ts` (no hand-authored mirror). MOCK ONLY ·
 * DRY-RUN ONLY · NO LIVE SEND · NO REAL CRM · NO PII — the banner below is shown
 * persistently. There are NO send / call / SMS / WhatsApp / ad controls anywhere
 * on this page.
 */
      ...
      <section style={card}>
        ...
        <ol style={{ ...muted, fontSize: 13, margin: '4px 0 0 18px' }}>
          {view.crm.timeline.map((e) => (
            <li key={e.id}>
              <code style={mono}>{e.kind}</code> · {e.outcome} ·{' '}
              <code style={mono}>{e.prospectId}</code>: {e.summary}
            </li>
          ))}
        </ol>
      </section>
