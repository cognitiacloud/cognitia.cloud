 ...
 * `@cognitia/agents` runs (the `budget_wheels_demo` / Tenant Zero sandbox):
 *
 ...
 * modules (the #159 integrated run packet + B4 audience + B5 TrustOps + B6 gates)
 * and returns the data rendered below. There is NO hand-authored structural
 * mirror of the lane logic, and no client component imports `@cognitia/agents`.
 *
 * MOCK ONLY · DRY-RUN ONLY · NO LIVE SEND · NO REAL CRM · NO PII — the banner is
 * shown persistently. There are NO send / call / SMS / WhatsApp / ad controls
 * anywhere on this page.
 */

import { loadCommandCenterData } from '../../lib/server/gtmCommandCenterData';
...
export default async function GtmCommandCenterPage() {
  ...
  return (
    <main style={{ maxWidth: 1040, margin: '0 auto', padding: 24, lineHeight: 1.55 }}>
      ...
      <p style={muted}>
        Integrated, end-to-end proof of the B1–B6 mock GTM system on one screen, assembled from real{' '}
        <code style={mono}>@cognitia/agents</code> runs: audience → assembly (compliance/approval) →
        dry-run channel engine → CRM-lite → TrustOps → release gates → proof/attribution. Tenant{' '}
      ...
      <section style={card}>
        ...
        {view.leads.map(({ id, console: c, channelPlan }) => (
          <div
            key={id}
