 ...
 * passes the threshold. The data is the REAL `@cognitia/agents` output assembled
 * by the server-only adapter. Uses `react-dom/server` in the node test env; no
 * browser/Playwright required. (JSX in the page is transformed via the automatic
 * runtime configured in `vitest.config.ts`.)
 */

import { describe, it, expect } from 'vitest';
import { renderToStaticMarkup } from 'react-dom/server';
import GtmCommandCenterPage from './page';

// The page is an async server component; await it, then render to static markup.
