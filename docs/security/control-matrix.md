...
> Status: ✅ implemented · 🟫 partial/wiring · ⛔ not yet.
>
> **Re-anchored 2026-06-10 to base `dc89123`** (HARD-4). The current,
> comprehensive source of truth is `docs/security/hardening-package-2026-06.md`
> (Deliverable 4) — read that first. This file is kept for CC-by-CC detail;
> the rows below were corrected to reflect the governed lifecycle merged
> through RUN-1. The prior `ea7677e` anchor predated session-derived auth and
> RBAC, so AC-2/AC-3 are now accurate (they previously, wrongly, read ⛔).
...
| AC-2 | CC6.1     | No client-trusted authz           | Tenant + role from a verified session principal (`HmacSessionVerifier`), **not** `x-tenant-id`; fail-closed without `SESSION_SECRET` | D1    | ✅ (session-derived)               | `serverAuth.test.ts`; smoke check 2        |
| AC-3 | CC6.3     | RBAC / least privilege (app)      | roles owner/operator/viewer; viewers can't approve/execute; owner-only resume                                                        | D1    | ✅                                 | `killSwitch.test.ts`; `serverAuth.test.ts` |
...
- **API-1 (session-derived auth) and RBAC are done (AC-2/AC-3 ✅).** The remaining gate before real customer data is now deploy-time identity (`app_user` role, KMS data key, TLS, backups) — see the hardening package gap #2.
