...
| `packages/agents`       | Agent runtime: run lifecycle, context builder, tool registry, policy gate, action ledger, guardrails, per-agent logic. | Direct external sends (must go through adapters + ledger). |
