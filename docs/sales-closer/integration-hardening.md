...
Unlike the web `gtmIntegratedDemoViewModel` (which structurally _reproduces_ lane
semantics because `apps/web` cannot depend on `@cognitia/agents`), this island
**actually calls** the tested lane modules, so it is the authoritative
