...
| Agent Fabric real remote execution                      | INTENTIONAL          | Fabric Lab is simulation-only; containment guard blocks process/network imports | deliberate migration + security sign-off                                        | none today; keep gated — no remote exec, no Tailscale/cloud yet    |
| Agent Fabric node attestation / reputation              | OPEN (by design)     | nodes are self-declared registry records; no real execution depends on them     | future design + gates                                                           | add signed attestation + node reputation before any real execution |
