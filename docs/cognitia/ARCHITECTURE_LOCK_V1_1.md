...
- ❌ On-chain contracts of any kind (ERC-8004 / EAS integration is a *later* posture, design notes only).
...
- **W3C Verifiable Credentials**: ATC data model is VC-*style* (issuer, subject, claims, issuance/expiry, status, proof section placeholder). We do not implement full VC cryptographic suites in v1.1; we keep the shape compatible.
...
- **No did:cognitia.** Agent identifiers are internal UUIDs/ULIDs plus optional standards-compatible external identifier fields (e.g. future ERC-8004 agent ID, future DID of an *existing* method).
...
- Agents/automation may *propose* `verified_fact`; promotion to `verified_fact` requires a verifier identity recorded on the proof (human or whitelisted automated verifier with its own audit trail).
