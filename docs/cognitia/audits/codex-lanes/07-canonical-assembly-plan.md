...
> undraft, edit, or otherwise mutate any PR. Every disposition below is a _recommendation_
...
concept is _also_ re-implemented in #125 and #128. This duplication is the single biggest
source of waste.
...
- **Recommended receipt _spec_ candidate: #140** (`docs/architecture/proof-receipt-spec.md`,
  13 required fields, redact-before-hash). Doc only.
- **Recommended receipt _implementation_ candidate: #144** (`@cognitia/gtm-os`): append-only
...
- **Runtime-lane decision** — standardize the spine on **TypeScript (`main`, pnpm/vitest)** or
  ...
  variant (#123). _Recommendation: TypeScript._ This is a founder gate; nothing here assumes
  the outcome.
...
- **#89** — investor audit whose central finding _contradicts_ the escrow/dispute/token thesis
...
> **Label:** _Proposed next-worker prompt after founder approval and canonical-lane decision._
