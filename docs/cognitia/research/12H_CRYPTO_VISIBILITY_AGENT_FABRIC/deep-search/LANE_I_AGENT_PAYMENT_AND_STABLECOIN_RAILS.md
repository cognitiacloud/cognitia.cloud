...
- `likely_inference` — Research is actively trying to **bind payment to verified
  ...
  proven — which is *exactly* Cognitia's escrow-releases-only-on-verified_fact model.
...
Cognitia's escrow + verified_fact release is the trust complement to x402's
payment rail: x402 moves money; Cognitia proves the work that *should* trigger the
