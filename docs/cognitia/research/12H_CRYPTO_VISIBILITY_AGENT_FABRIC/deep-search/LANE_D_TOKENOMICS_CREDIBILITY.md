...
| Credible | Scammy |
| -------- | ------ |
| Token exists because the protocol *needs* it | Token-first, utility "later" |
...
- `likely_inference` — The most credible token utility for a trust/work protocol
  ...
  workers/arbiters — value that must be *at risk* to mean anything.
...
Cognitia's tokenomics credibility comes from *restraint*: it has a working
