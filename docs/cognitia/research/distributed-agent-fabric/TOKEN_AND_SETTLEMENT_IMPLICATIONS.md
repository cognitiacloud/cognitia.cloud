...
## Why distribution *might* later motivate a token (still gated)
...
- The credible answer is **assurance collateral**: a node/verifier/publisher posts
  ...
  *at risk* to mean something — which internal, non-transferable credits cannot
...
Cross-tenant *real* settlement is itself gated (legal + AML).
